        ...
        <div className="space-y-12">
          <section>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Mission Briefing</h2>
              {isCompleted && <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 text-xs font-bold rounded-full border border-emerald-500/20">COMPLETED</span>}
            </div>
          ...
          <div className="p-8 rounded-3xl bg-[#161b22] border border-[#30363d] shadow-xl">
            ...
            {localJob.status === 'Open' && (
              <div className="space-y-3">
                <button
                  onClick={async () => {
                    await fetch(`http://localhost:4000/api/v1/jobs/${localJob.id}/apply`, {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                ...
                <button
                  onClick={async () => {
                    ...
                    await fetch(`http://localhost:4000/api/v1/jobs/${localJob.id}/fund`, {
                      method: 'POST',
            ...
            {localJob.status === 'Completed' && ( // Should check Escrow status too
              <button
                onClick={async () => {
                  ...
                  await fetch(`http://localhost:4000/api/v1/jobs/${localJob.id}/release`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
              ...
              >
                <span className="material-symbols-outlined text-sm align-bottom mr-2">payments</span>
                Release Payment
              </button>
            )}
