...
const StatCard = ({ label, value, subtext, icon, trend }: any) => (
    <div className="bg-[#1e1e2e] border border-white/5 rounded-xl p-6 relative overflow-hidden group">
        ...
        <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">{label}</h3>
        <div className="text-4xl font-black text-white mb-2">{value}</div>
        <div className={`text-xs font-bold ${trend.includes('+') ? 'text-emerald-400' : 'text-amber-400'}`}>
            {trend} {subtext}
        </div>
    </div>
);

const ProofProfileModal: React.FC<ProofProfileModalProps> = ({ isOpen, onClose, onProceed, profile }) => {
            ...
            <div className="w-full h-full max-w-[1400px] flex overflow-hidden bg-[#0d0d12] relative border border-white/5 rounded-2xl shadow-2xl my-8 md:my-12 mx-4 md:mx-12">
                ...
                <div className="flex-1 flex flex-col min-w-0 overflow-y-auto custom-scrollbar p-10">
                    ...
                    <header className="flex items-center justify-between mb-12">
                        <div className="flex items-center gap-6">
                            <div className="size-24 rounded-2xl p-[2px] bg-gradient-to-br from-purple-500 to-cyan-500">
                                <img src={profile.avatar} className="size-full rounded-2xl object-cover border-4 border-[#0d0d12]" alt="Avatar" />
                            </div>
                            <div>
                                <div className="flex items-center gap-2 mb-1">
                                    <h1 className="text-3xl font-black text-white tracking-tight">{profile.name}</h1>
                                    <span className="material-symbols-outlined text-emerald-400 filled !text-xl">verified</span>
                                </div>
                                <div className="flex items-center gap-4 text-sm">
                                    <span className="text-slate-400 font-medium">Senior Full-Stack Engineer</span>
                        ...
                        <div className="flex gap-4">
                            <button className="px-6 py-2.5 bg-white/5 hover:bg-white/10 border border-white/5 rounded-lg text-white text-xs font-bold uppercase tracking-widest transition-all">
                                View GitHub
                            </button>
                            <button className="px-6 py-2.5 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-xs font-bold uppercase tracking-widest shadow-lg shadow-purple-600/20 transition-all">
                                Download Resume
                            </button>
                        </div>
                    </header>
                    ...
                    <div className="grid grid-cols-3 gap-6 mb-10">
                        <StatCard label="Production Merges" value="12.4k" trend="↗ +12%" subtext="vs avg" icon="commit" />
                        <StatCard label="Review Turnaround" value="4.2h" trend="↘ -15%" subtext="(faster)" icon="timer" />
                        <StatCard label="Arch Approvals" value="18" trend="↗ +5%" subtext="quarterly" icon="architecture" />
                    </div>

                    {/* Heatmap Section */}
                    <div className="bg-[#1e1e2e] border border-white/5 rounded-xl p-8 mb-10">
                        <div className="flex items-center justify-between mb-8">
                            <h3 className="text-lg font-bold text-white">3D Production Impact Heatmap</h3>
                            <div className="flex gap-4 text-xs font-bold">
                                <div className="flex items-center gap-2"><span className="size-2 rounded-full bg-slate-700"></span> Low Impact</div>
                                <div className="flex items-center gap-2"><span className="size-2 rounded-full bg-purple-500"></span> High Impact</div>
                            </div>
                        </div>
                        <div className="h-64 flex items-end justify-between px-4 gap-2">
                            ...
                            {Array.from({ length: 48 }).map((_, i) => {
                                ...
                                return (
                                    <div key={i} className="flex-1 bg-purple-500 rounded-t-sm transition-all hover:bg-purple-400" style={{ height: `${height}%`, opacity }}></div>
                                );
                            })}
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-10">
                        ...
                        <div className="space-y-6">
                            <h3 className="text-lg font-bold text-white">Architectural Skill Clusters</h3>
                            ...
                            <div className="bg-[#1e1e2e] border border-white/5 rounded-xl p-6">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <span className="font-bold text-white">Distributed Systems</span>
                                </div>
                                ...
                                <div className="flex gap-2">
                                    {['Microservices', 'gRPC', 'Kafka'].map(tag => (
                                        <span key={tag} className="px-2 py-1 bg-cyan-500/10 text-cyan-400 text-[10px] font-bold rounded uppercase tracking-wider border border-cyan-500/20">{tag}</span>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="bg-[#1e1e2e] border border-white/5 rounded-xl p-6">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <span className="font-bold text-white">Frontend Performance</span>
                                </div>
                                <div className="h-2 bg-slate-800 rounded-full overflow-hidden mb-4">
                                ...
                                <div className="flex gap-2">
                                    {['Next.js Core', 'WASM', 'V8 Optimization'].map(tag => (
                                        <span key={tag} className="px-2 py-1 bg-purple-500/10 text-purple-400 text-[10px] font-bold rounded uppercase tracking-wider border border-purple-500/20">{tag}</span>
                                    ))}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-6">
                            <div className="bg-[#1e1e2e] border border-white/5 rounded-xl p-6">
                                <h3 className="text-sm font-bold text-white mb-6">Tech Provenance</h3>
                                    ...
                                    {[
                                        ...
                                        <div key={tech.name}>
                                            <div className="flex justify-between text-xs mb-2">
                                                ...
                                                <span className="text-slate-500 font-mono text-[10px]">{tech.val}% impact</span>
                                            </div>
                                            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                                                <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${tech.val}%` }}></div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="mt-auto bg-gradient-to-br from-purple-600 to-indigo-600 rounded-2xl p-8 text-center border border-white/10 shadow-2xl relative overflow-hidden">
                                ...
                                <div className="relative z-10">
                                    <h2 className="text-2xl font-black text-white mb-2">Hire {profile.name.split(' ')[0]}</h2>
                                    <p className="text-purple-100 text-sm mb-6 leading-relaxed max-w-[80%] mx-auto">
                                        {profile.name.split(' ')[0]} is currently considering high-impact roles in Distributed Systems or FinTech.
                                    </p>
                                    <button
                                        onClick={onProceed}
