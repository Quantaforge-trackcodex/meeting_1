...
const OfferJobModal: React.FC<OfferJobModalProps> = ({ isOpen, onClose, targetUser }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    reportingManager: 'Alex Rivera (Engineering VP)',
  ...
  if (showAcceptancePreview && createdJob) {
    return <JobAcceptanceModal isOpen={true} onClose={onClose} offer={createdJob} />;
  }
      ...
      <div className="bg-[#0d1117] border border-[#30363d] w-full max-w-5xl h-[85vh] rounded-2xl overflow-hidden shadow-2xl flex">
        ...
        <div className="w-64 border-r border-[#30363d] bg-[#010409] p-6 hidden md:flex flex-col">
          <div className="flex items-center gap-3 mb-8">
            <div className="size-8 rounded-lg bg-emerald-500/20 flex items-center justify-center text-emerald-500 font-bold">A</div>
            <div className="text-white font-bold leading-tight">
              {targetUser.name}<br />
              <span className="text-[10px] text-slate-500 font-normal">Candidate</span>
            </div>
          </div>

          <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Approval Workflow</h3>
          <div className="space-y-6 relative">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-[10px] text-emerald-500">Approved by Sarah K.</div>
              </div>
            </div>

            <div className="flex items-start gap-3">
              <div className="size-5 rounded-full bg-blue-500 flex items-center justify-center mt-0.5 ring-4 ring-[#010409] animate-pulse">
                <span className="material-symbols-outlined !text-[12px] text-white">schedule</span>
              </div>
              <div>
                <div className="text-xs font-bold text-white">Dept Head Approval</div>
                <div className="text-[10px] text-blue-400">Current Stage</div>
              </div>
            </div>

            <div className="flex items-start gap-3 opacity-50">
              <div className="size-5 rounded-full bg-[#30363d] flex items-center justify-center mt-0.5 ring-4 ring-[#010409]">
              </div>
              <div>
                <div className="text-xs font-bold text-white">Finance Verification</div>
        ...
        <div className="flex-1 flex flex-col bg-[#0d1117] min-w-0">
          <div className="h-16 border-b border-[#30363d] flex items-center justify-between px-6 bg-[#161b22]">
            <h2 className="text-lg font-bold text-white">Offer Details & Terms</h2>
            <div className="flex items-center gap-3">
              <button onClick={onClose} className="px-4 py-2 hover:bg-[#30363d] text-slate-400 hover:text-white rounded-lg text-xs font-bold transition-colors">Save Draft</button>
              <button form="offer-form" type="submit" className="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-bold shadow-lg shadow-blue-600/20 transition-all flex items-center gap-2">
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            <form id="offer-form" onSubmit={handleSubmit} className="max-w-2xl space-y-8">

              <section>
                <h3 className="text-xs font-black text-blue-400 uppercase tracking-widest mb-4">Compensation</h3>

                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-2">Base Salary (Annual)</label>
                ...
                <div className="grid grid-cols-2 gap-6 mt-6">
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-2">Equity (Options)</label>
                    <input
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-2">Sign-on Bonus</label>
                    <input
                      className="w-full bg-[#161b22] border border-[#30363d] rounded-lg px-3 py-2.5 text-sm text-white focus:ring-1 focus:ring-blue-500 outline-none transition-all"
              ...
              <section>
                <h3 className="text-xs font-black text-blue-400 uppercase tracking-widest mb-4">Logistics</h3>

                <div className="space-y-6">
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-2">Start Date</label>
                    <input
                      type="date"
                      className="w-full bg-[#161b22] border border-[#30363d] rounded-lg px-3 py-2.5 text-sm text-white focus:ring-1 focus:ring-blue-500 outline-none transition-all"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-2">Reporting Manager</label>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-2">Office Location</label>
                    <div className="relative">
                      <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 !text-[18px]">location_on</span>
                      <input
                        className="w-full bg-[#161b22] border border-[#30363d] rounded-lg pl-10 pr-3 py-2.5 text-sm text-white focus:ring-1 focus:ring-blue-500 outline-none transition-all"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-2">Prerequisites / Requirements</label>
                    <textarea
              ...
              <section className="bg-[#161b22] rounded-xl p-4 border border-[#30363d] space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold text-white">Include Relocation Package</span>
                  <button
                    type="button"
                    onClick={() => setFormData({ ...formData, includeRelocation: !formData.includeRelocation })}
                    className={`w-10 h-5 rounded-full relative transition-colors ${formData.includeRelocation ? 'bg-blue-600' : 'bg-[#30363d]'}`}
                  >
                    <div className={`absolute top-1 left-1 size-3 bg-white rounded-full transition-transform ${formData.includeRelocation ? 'translate-x-5' : ''}`}></div>
                  </button>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold text-white">Custom NDA Required</span>
                  <button
                    type="button"
                    onClick={() => setFormData({ ...formData, customNDA: !formData.customNDA })}
                    className={`w-10 h-5 rounded-full relative transition-colors ${formData.customNDA ? 'bg-blue-600' : 'bg-[#30363d]'}`}
                  >
                    <div className={`absolute top-1 left-1 size-3 bg-white rounded-full transition-transform ${formData.customNDA ? 'translate-x-5' : ''}`}></div>
                  </button>
                </div>
              </section>
